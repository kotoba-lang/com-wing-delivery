(ns wing_delivery.main
  "Kotodama WASM entrypoint for the Wing_delivery clean-room actor (L4) — Clojure port.
  ...
  facts (`wing_delivery.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wing_delivery")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "wingdeli_dro"
    :fields [:serial :model :batteryPct :status] :required [:serial :model]
    :coerce {:batteryPct :float}       :refs {}}
   {:entity "Mission"    :plural "missions"    :id-prefix "wingdeli_mis"
    :fields [:droneId :type :status]         :required [:type :status]
    :coerce {}                         :refs {:droneId "Drone"}}
   {:entity "Waypoint"   :plural "waypoints"   :id-prefix "wingdeli_way"
    :fields [:missionId :lat :lon :altitudeM :seq] :required [:lat :lon]
    :coerce {:lat :float :lon :float :altitudeM :float :seq :int} :refs {:missionId "Mission"}}
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "wingdeli_tel"
    :fields [:droneId :lat :lon :altitudeM :recordedAt] :required [:lat :lon]
    :coerce {:lat :float :lon :float :altitudeM :float} :refs {:droneId "Drone"}}
   {:entity "Payload"    :plural "payloads"    :id-prefix "wingdeli_pay"
    :fields [:droneId :type :weightKg]       :required [:type :weightKg]
    :coerce {:weightKg :float}         :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "wingdeli_swa"
    :fields [:name :droneCount :leaderId]    :required [:name :droneCount]
    :coerce {:droneCount :int}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"wing_delivery.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wing_delivery-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wing_delivery-compat/ready)
